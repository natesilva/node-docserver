...
// extensions (Array) - Extensions (including leading dot) to try
//  appending to the target name
...
function findMatch(target, extensions, callback) {
  ...
  if (!callback && typeof(extensions) === 'function') {
    callback = extensions;
    extensions = null;
  }

  if (!extensions) { extensions = ['.md', '.mdown']; }
