...
// memoization wrapper for render()

function cachedRender(target, options, callback) {
  var cacheKey = 'render:' + target;
  options.cache.get(cacheKey, function(err, value) {
    if (err) { callback(err); }
    else if (value) { callback(null, value); }
    else { render(target, options, function(err, html) {
      if (err) { return callback(err); }
      options.cache.set(cacheKey, html);
      callback(null, html);
    }); }
  });  
}

module.exports = cachedRender;
